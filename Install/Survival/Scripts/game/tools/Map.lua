...
function Map.sv_PreLoadWayPoints( self )
	print("Locating Host...")
	self.network:sendToClients("FindHost", sm.player.getAllPlayers()[#sm.player.getAllPlayers()]:getName())
end
...
function Map.PreLoadWayPoints( self, params )
	...
	for i,v in pairs(GuiTablePos) do
		...
		GuiTable[i]:setImage("Icon", "$SURVIVAL_DATA/Scripts/WayPoints/WayPointIcon"..tostring(Color[i])..".png")
		GuiTable[i]:open()
	end
end
...
function Map.client_addGui( self, params )
	...
	GuiTable[Map.NumberOfWaypoints]:setImage("Icon", "$SURVIVAL_DATA/Scripts/WayPoints/WayPointIcon"..tostring(Map.Color)..".png")
...
function Map.cl_addGui( self, params )
	...
	if params.PlayerName ~= tostring(sm.localPlayer.getPlayer():getName()) then
		...
		GuiTable[params.i]:setImage("Icon", "$SURVIVAL_DATA/Scripts/WayPoints/WayPointIcon"..tostring(params.color)..".png")
...
function Map.SaveDataForAll( self, params )
	params.name = sm.player.getAllPlayers()[#sm.player.getAllPlayers()]:getName()
...
function Map.client_onUpdate( self, dt )
	...
	elseif tmpHasBeenRun then
		if Map.LoadCellsForGUIPause == 10 then
			self:client_addGui()
			tmpHasBeenRun = false
			Map.LoadCellsForGUIPause = 0
		end
	...
	if Map.SaveData then
		if Map.MapLevel > 1 or GetGameMode() then
			...
			self.network:sendToServer("SaveDataForAll", { pos = GuiTablePos, color = GuiColor, level = Map.MapLevel,  })
		end
		Map.SaveData = false
	end
	...
	if Map.ToggleOnOff and Map.AnimationEnded then
		isSprinting = true
	elseif not self.tool:isSprinting() and not Map.ToggleOnOff and Map.AnimationEnded and self.equipped then
		isSprinting = false
	...
	for i,v in pairs(GuiTablePos) do
		if (GuiTablePos[i] ~= nil) then
			...
			local StartHeight = (DistanceRound/75+1)
			...
			GuiTableDistance[i]:setMaxRenderDistance(10000)
...
function Map.client_onEquippedUpdate( self, primaryState, secondaryState, data0, data1, data2 )
	...
	if primaryState == 0 and Map.Ready == true and Map.ToggleOnOff and Map.AnimationEnded then
		...
		cl_MapGuiOn( self, inventory, player )
		Map.MapLevel = GetMapLevel()
		params = { location = Map.BlockPos, direction =  Map.BlockDir}
